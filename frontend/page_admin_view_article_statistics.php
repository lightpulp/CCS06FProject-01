...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['change_status']) && isset($_POST['article_id'])) {
    ...
    $update_id = intval($_POST['article_id']);

    $update_sql = "UPDATE articles SET status = ? WHERE article_id = ?";
    $update_stmt = $conn->prepare($update_sql);
    $update_stmt->bind_param("ii", $new_status, $update_id);
                ...
                <div class="row">
                            ...
                            <div class="col-12">
                                <div>
                                    ...
                                    <div class="d-flex justify-content-between align-items-center mb-3">
                                        ...
                                        <a href="page_admin_article_management.php" class="text-decoration-none text-muted">
                    ...
                    <div class="col-12 col-md-4 mt-4 mt-md-0">
                        <div class="position-md-sticky" style="top: 1rem;">
                            ...
                            <!--- Change Article Staus --->
                            <div class="mb-3 border-bottom">
                                <h6 class="text-muted fw-bold fs-6">Change Article Status</h6>

                                <form method="post" action="" class="d-flex flex-wrap gap-2">
                                    <input type="hidden" name="article_id" value="<?php echo $article['article_id']; ?>">
                                    <button type="submit" name="change_status" value="1" class="btn btn-warning btn-sm">Set to Pending</button>
                                    <button type="submit" name="change_status" value="2" class="btn btn-success btn-sm">Approve</button>
                                    <button type="submit" name="change_status" value="3" class="btn btn-danger btn-sm">Flag as Fake</button>
                                </form>

                                <?php
                                // Show current reason text
                                echo '<p class="small text-muted mt-3 fs-7">';
                                if ($article['status'] == 1) {
                                    echo 'Article is under review and waiting for approval.';
                                } elseif ($article['status'] == 2) {
                                    echo 'Article has been approved and published.';
                                } elseif ($article['status'] == 3) {
                                    echo 'Article has been flagged as potentially containing misinformation.';
                                } else {
                                ...
                                <div class="mb-3 border-bottom">
                                    ...
                                    <a href="page_admin_edit_article.php?id=<?php echo $article['article_id']; ?>" class="btn btn-primary w-100 mt-3 primaryBtnAnimate">Edit Article</a>
                                </div>
