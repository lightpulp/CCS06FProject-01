...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($article_id && $title && $content && $category_id && $date_published) {
        ...
        $stmt = $conn->prepare("UPDATE articles SET title = ?, content = ?, source_url = ?, category_id = ?, date_published = ?, percentage = ?, f_words = ?, f_score = ?, total_words = ? WHERE article_id = ? AND user_id = ?");
        $stmt->bind_param("sssisdsiiii", $title, $content, $source_url, $category_id, $date_published, $formattedPercentage, $fWordsList, $fakeScore, $totalWords, $article_id, $user_id);
